...
#'
#' @description calculates deprivation score for item j and ind. i as $d_{j,i} = (1-F(c_{j,i})) / (1-F(1)) where F(c_{j,i})$ is the value of j-th item cumulation function for the i individual
...
fuzzyScaleItem = function(item, weight, ID, ...){
  ...
  outW2 = outW %>% 
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(d = (1 - F_cum) / (1 - min(F_cum)), 
                  s = 1 - d,
                  Item = item) %>%
