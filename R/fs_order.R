...
#'
#' @description Detect and invert deprivation items for FS
...
fs_order <- function(data, vec_order){
  ...
  for (i in 1:ncol(data)){
    ...
    extract=unique.numeric_version(data[,i])
    if (vec_order[i]==TRUE) {dataex=data.frame("extract"=extract, "new_order"=sort(extract, vec_order[i]))
    }     else{dataex=data.frame("extract"=extract, "new_order"=extract) }
    newdata[,i]=data[,i]
