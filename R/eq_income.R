...
#' eq_income(monetary=aa, ncomp=ncomp, age=age16, scale.eq="OECD7050") #OECD7050
#' eq_income(monetary=aa, ncomp=ncomp, age=age16, scale.eq="modifiedOECD") #modifiedOECD
...
eq_income = function(monetary, ncomp, age, scale.eq,newscale){
  if (scale.eq=="carbonaro"){
    ...
    eq_income=data.frame("monetary"=monetary, "ncomp"=ncomp, "age"=age, "s.eq"=(1+(ncomp-1-age)*0.7+age*0.5))
    eq_income$monetary.eq= eq_income$monetary/ eq_income$s.eq
  } else if (scale.eq=="modifiedOECD"){
    eq_income=data.frame(  "monetary"=monetary, "ncomp"=ncomp, "age"=age,"s.eq"=(1+(ncomp-1-age)*0.5+age*0.3))
    eq_income$monetary.eq= eq_income$monetary/ eq_income$s.eq
