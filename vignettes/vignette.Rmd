...
```{r}
FL.curve = fm_FL(verma$results$predicate, verma$results$weight)
verma.plot.data = data.frame(verma$results, Lorenz = FL.curve$Lorenz, WECDF = FL.curve$WECDF)
verma.plot.data %>% dplyr::select(-weight) %>% 
  tidyr::pivot_longer(cols = c(mu, Lorenz, WECDF), names_to = "curve", values_to = "value") %>%
  ggplot(aes(x = predicate, y = value, linetype = curve)) +
  geom_line(area = "black") + 
  scale_linetype_manual(values = c("Lorenz" = "dashed", "mu" = "solid", "WECDF" = "dotted")) +
  geom_area(position = "identity", alpha = .1) +
  scale_x_continuous("Predicate", labels = function(x) format(x, scientific = FALSE), n.breaks = 6) + 
...
```{r}
hh.size = sample(1:4, length(eusilc$HB020), replace = T, prob = c(4,3,2,1))
zbm = fm_construct(predicate = eusilc$eq_income, weight = eusilc$DB090, fm = "ZBM", hh.size = hh.size)

data.frame(ID = zbm$results$ID, predicate = zbm$results$predicate, zbm$mu_k) %>% tidyr::pivot_longer(dplyr::contains("X"), names_to = "mu", values_to = "value") %>% 
  ggplot2::ggplot(aes(x = predicate, y = value)) + geom_line() + geom_area(alpha = .2) + facet_wrap(~mu) + 
    scale_x_continuous("Predicate", labels = function(x) format(x, scientific = FALSE), n.breaks = 6) + 
  scale_y_continuous(expression(mu)) +
  theme_minimal() + 
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5),
...
```{r}
par.df = data.frame(Parameters = names(belhadj$parameters[-4]),
                    value = unlist(belhadj$parameters[-4]),
                    y = c(0))
par.lab = bquote(.(belhadj$parameters[4]))
ggplot(belhadj$results, aes(x = predicate, y = mu)) + 
  geom_line(size = .8) + 
  scale_x_continuous("Predicate", labels = function(x) format(x, scientific = FALSE), n.breaks = 6) + 
  scale_y_continuous(expression(mu)) +
  geom_vline(aes(xintercept = value, colour = Parameters), data = par.df, linetype = "dashed") +
...
```{r}
par.df = data.frame(par = names(chakravarty$parameters),
                    value = unlist(chakravarty$parameters),
                    y = c(0))
ggplot(chakravarty$results, aes(x = predicate, y = mu)) + 
  geom_line() + 
  scale_x_continuous("Predicate") + 
  scale_y_continuous(expression(mu)) +
  geom_vline(aes(xintercept = value, colour = par), data = par.df, linetype = "dashed") +
  theme(axis.title.y = element_text(angle = 0, vjust = 0.5))
```

```{r}
summary(chakravarty)
plot(chakravarty$results$predicate, chakravarty$results$mu, type = "l")
```
