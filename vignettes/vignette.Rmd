...
verma = fm_construct(predicate = eusilc$eq_income, weight = eusilc$DB090, ID = NULL,
                     HCR = hcr, interval = c(1,20), alpha = NULL, fm = "verma1999", verbose = FALSE)
verma$fm
class(verma)
summary(verma)
plot(verma)
...
```{r}
verma.break$results %>% ggplot(aes(x = predicate, y = mu)) + 
  geom_line() + geom_area(alpha = .2) + 
...
data.frame(ID = zbm$results$ID, predicate = zbm$results$predicate, zbm$mu_k) %>% 
  tidyr::pivot_longer(dplyr::contains("X"), names_to = "mu", values_to = "value") %>% 
...
```{r}
par.df = data.frame(Parameters = names(chakravarty$parameters),
                    value = unlist(chakravarty$parameters),
                    y = c(0))
ggplot(chakravarty$results, aes(x = predicate, y = mu)) + 
  geom_line() + geom_area(alpha = .1) + 
...
```{r eval=FALSE}
FS$membership$FS1
FS$membership$FS1 %>% ggplot(aes(x = s_hi, y = mu)) + geom_line() + facet_wrap(~breakdown)

# no breakdown
FS$membership$Overall %>% ggplot(aes(x = s_i, y = mu)) + geom_line()
FS$membership$FS5 %>% ggplot(aes(x = s_hi, y = mu)) + geom_line()
```
