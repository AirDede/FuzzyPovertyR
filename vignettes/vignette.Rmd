...
steps4_5$steps4_5 %>% group_by(Item, Dimension) %>% 
         summarise(Mean = mean(s)) %>% arrange(Dimension) %>%
  ggplot(aes(x = reorder(Item, Dimension), y = Mean)) + 
  geom_bar(aes(fill = factor(Dimension)), color = "black", stat = "identity", alpha = .7) + 
  geom_line(aes(x = Item, y = value, group = weight, linetype = weight), 
            data = steps4_5$steps4_5 %>% group_by(Item) %>% 
         summarise(
                   w = mean(w),
                   w_a = mean(w_a),
                   w_b = mean(w_b)) %>% 
         pivot_longer(cols = c(w, w_a, w_b), names_to = "weight", values_to = "value")) +
  theme_minimal() + 
  scale_linetype_manual(values = c("solid", "dotted", "dashed"), labels = expression(w, w[a], w[b]))+
...
steps4_5$steps4_5 %>% 
  ungroup() %>%
  select(Item, Dimension, w) %>% 
  distinct() %>%
  ggplot(aes(x = Dimension, y = w, fill = reorder(Item, Dimension))) + 
  # coord_polar() +
...
```{r}
fm_var(predicate = eusilc$eq_income, weight = eusilc$DB090, fm = "verma",  type = "bootstrap", HCR = .14, alpha = alpha, verbose = T)
...
```{r eval=FALSE}
variance = fs_var(data = eusilc[,4:23], weight = eusilc$DB090, ID = NULL, dimensions = dimensions, breakdown = NULL, HCR = 0.15, alpha = 5, rho = NULL, type = 'bootstrap', M = NULL, R = 2, verbose = F)
summary(variance)
```

```{r}
fs_var(data = eusilc[,4:23], weight = eusilc$DB090, ID = NULL, dimensions = dimensions, breakdown = eusilc$db040, HCR = .16, alpha = 2, rho = NULL, type = 'bootstrap', M = NULL, R = 2, verbose = F) %>% summary()
```
...
fs_var(data = eusilc[,4:23], weight = eusilc$DB090, ID = NULL, dimensions = dimensions, 
       breakdown = eusilc$db040, HCR = .16, alpha = 2, rho = NULL, type = 'jackknife', 
